...
import com.industrialscansystem.Controller.util.SaveUploadFile;
...
//    }
    ...
    public void uploadFileList(@PathParam("uploadFile") MultipartFile[] uploadFile,
        ...
        System.out.println("tomcatPath:" + tomcatPath);//设置的全局变量，省的每次都改，嘻嘻  <------- 牛的
        ...
        for (MultipartFile multipartFile : uploadFile) {
            ...
            } else//非TIFF格式图片
            {
                ...
                if (index < pictureList.size()) {
                    Picture p = pictureList.get(index);
                    if (p.getPicture_dir().equals("空")) {
                        ...
                        p.setPicture_width(sourceImg.getWidth());
...
//            }
    ...
    @RequestMapping(value = "/deleteRequisitionById/{requisition_id}/{product_id}")
    public List<Requisition> deleteRequisitionById(@PathVariable("requisition_id") int requisition_id, @PathVariable("product_id") int product_id) {
        int p_id = product_id;
        Requisition requisition = requisitionRespository.getRequisitionById(requisition_id);
        ...
        return requisitionRespository.selectRequisitionListByproductId(p_id);
    }
    ...
    @RequestMapping(value = "/getRequisitionListByProductId/{product_id}")
    public Object getRequisitionListByProductId(@PathVariable("product_id") int product_id) {
        List<Requisition> requisitionList = requisitionRespository.selectRequisitionListByproductId(product_id);
...
//            requisition.setRequisition_entrytime(date);
            ...
            Requisition r = requisitionRespository.save(req);
            int id = r.getRequisition_id();
            res[0] = "申请单已添加成功！请前往导入影像图";
            res[1] = String.valueOf(id);
            return res;
        }
        res[0] = "已有同编号申请单，请勿重复添加";
        res[1] = String.valueOf(-1);
        return res;
    }
