...
public class LogSaveService {
    ...
    public void LogInfoSave(String operation){ //记录存储log用户信息服务

        String tempUsername = SecurityContextHolder.getContext().getAuthentication().getName();//获取当前登录的用户信息
        Member member = memberRespository.findMemberByMember_username(tempUsername);
        //取得登录的用户姓名
        String memberName = member.getMember_name();
        //取得用户的当前权限
        int memberRole = member.getMember_role();
        //取得现在的操作时间
