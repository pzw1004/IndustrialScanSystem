...
public interface RetangleRespository extends JpaRepository<Retangle,Integer> {
    ...
    @Modifying
    @Query(value = "delete  from  retangle  where retangle_id=?1 ",nativeQuery = true)
     void deleteRetangleById(Integer retangle_id);

    @Modifying
    @Query(value = "delete  from  retangle   where retangle_picture_id=?1 and retangle_author = 'Algorithm'",nativeQuery = true)
    public void deleteAIRetangleListByPictureId(Integer retangle_picture_id);
    ...
    @Query(value = "select * from retangle where retangle_id=?1 ",nativeQuery = true)
    public Retangle getRetangleByRetangleId(Integer retangle_id);
